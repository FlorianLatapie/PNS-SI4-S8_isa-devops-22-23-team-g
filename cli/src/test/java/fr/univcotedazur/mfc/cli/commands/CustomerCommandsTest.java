...
class CustomerCommandsTest {
    ...
    void customersTest(){
        ...
        server
                .expect(requestTo("/customers/login"))
                .andExpect(method(HttpMethod.POST))
                .andRespond(withSuccess("{\"name\":\"test\", \"status\": \"CLASSIC\",\"licensePlate\":\"123456\"}", MediaType.APPLICATION_JSON));

        server
                .expect(requestTo("/customers/login"))
                .andExpect(method(HttpMethod.POST))
                .andRespond(withSuccess("{\"name\":\"test2\", \"status\": \"CLASSIC\", \"licensePlate\":\"123456\"}", MediaType.APPLICATION_JSON));
        client.login("test");
        client.login("test2");

        assertEquals("{test2=CliCustomer{id=null, name='test2', points=0, euros=0.0, status=CLASSIC, licensePlate=123456}, test=CliCustomer{id=null, name='test', points=0, euros=0.0, status=CLASSIC, licensePlate=123456}}", client.customers());
    }
}
